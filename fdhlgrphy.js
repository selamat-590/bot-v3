...
//-- seberapa gay
                ...
                if (!q) return reply(`Penggunaan	 ${command} text\n\nContoh : ${command} Rara`)
                                axios.get(`https://arugaz.herokuapp.com/api/howgay`).then(res => res.data).then(res =>                                                                                                                                                                                                          textImg(`Nih Liat Data Gay Si ${q}\n\nPersentase Gay : ${res.persen}%\nAlert!!! : ${res.desc}`))
                                break
//-- ganteng
				case prefix+'ganteng':
                                      if (args.length < 1) return reply(`Contoh : ${prefix}gantengcek kaysan`)
				      if (!isGroup) return reply(mess.only.group)
 					 random = `${Math.floor(Math.random() * 100)}`
  					gan = q
  					cek = `Target : *${gan}*
...
//-- cantik
                                case prefix+'cantik':
                                        ...
                                        fdhl.sendMessage(form, vejs, text, {quoted: mek})
                                        break
//-- jadian
                                        case prefix+'jadian':
